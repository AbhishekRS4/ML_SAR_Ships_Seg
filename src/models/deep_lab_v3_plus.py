...
class DeepLabV3Plus(nn.Module):
    ...
    encoder_projection_out_channels : int
    ...
    def __init__(
        ...
        self.projection_conv = nn.Sequential(
            nn.Conv2d(
                encoder_projection_in_channels,
                encoder_projection_out_channels,
                1,
                bias=False,
            ),
            nn.GroupNorm(1, encoder_projection_out_channels),
            nn.GELU(),
        )
        ...
        self.classifier_conv_block = nn.Sequential(
            nn.Conv2d(
                aspp_out_channels + encoder_projection_out_channels,
                final_out_channels,
                3,
                padding=1,
                bias=False,
            ),
            nn.GroupNorm(1, final_out_channels),
            nn.GELU(),
...
class ASPPBlock(nn.Module):
    def __init__(self, in_channels, atrous_rates, aspp_out_channels=256):
        ...
        self.aspp_init_conv = nn.Sequential(
            ...
            nn.GroupNorm(1, aspp_out_channels),
            nn.GELU(),
        )
