...
from models.polarized_self_attention import PSA_s
from models.model_layer_utils import get_activation_func


class PSABasicBlock(nn.Module):
    expansion: int = 1
...
class CustomPSAResNet(nn.Module):
    def __init__(
        self,
        num_input_feats: int,
        layers: List[int],
        block=Union[PSABasicBlock, Bottleneck],
        groups: int = 1,
        ...
        super(CustomPSAResNet, self).__init__()
...
def _psa_resnet(
    num_input_feats: int,
    block_type: Union[PSABasicBlock, Bottleneck],
    layers: List[int],
    activation: str = "gelu",
    filters: List[int] = [64, 128, 256, 512],
) -> CustomPSAResNet:
    ...
    Returns
    -------
    model : CustomPSAResNet
        model object of type CustomPSAResNet
    """
    model = CustomPSAResNet(
