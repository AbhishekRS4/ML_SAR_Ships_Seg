...
def train_nn_ddp_dali(
    ...
    train_metrics: SemSegMetricsCalculator,
    ddp_context: DDPContext,
) -> tuple:
    """
    ...
    train_loader: DALIGenericIterator
        train data loader object (sharded for this local_rank)
    criterion: Union[CrossEntropyLoss, FocalLoss]
    ...
    ddp_context: DDPContext
        DDPContext object containing important params
    ...
    running_train_loss = torch.zeros(1, device=ddp_context.device)
    train_metrics.reset_metrics()

    for batch in train_loader:
        image = batch[0]["images"].float()
        label = batch[0]["labels"].long()
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            train_metrics.update_metrics(label, pred_label)
    ...
    train_loss = reduce_tensor(train_loss, ddp_context.world_size)
...
def test_nn_ddp_dali(
    ...
    test_loader,
    criterion: Union[CrossEntropyLoss, FocalLoss],
    test_metrics: SemSegMetricsCalculator,
    ddp_context: DDPContext,
) -> tuple:
    """
    ...
    test_loader: DALIGenericIterator
        test data loader object (sharded for this local_rank)
    criterion: Union[CrossEntropyLoss, FocalLoss]
        criterion object for the loss function
    test_metrics: SemSegMetricsCalculator
        object for the metrics calculator
    ...
    running_test_loss = torch.zeros(1, device=ddp_context.device)
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            test_metrics.update_metrics(label, pred_label)

            running_test_loss += loss

    # reduce loss across all ranks
    test_loss = running_test_loss / num_test_batches
    test_loss = reduce_tensor(test_loss, ddp_context.world_size)
...
def train_sem_seg_ddp_pipeline(
    ...
    experiment_name: str = "HRSID_SAR",
    run_name: str = "HRSID_SAR_DDP",
    model_name: str = "convnext_v2_tiny_deeplab_v3+",
    loss_fn: str = "focal",
    ...
    model_compile_mode: str = "uncompiled",
    file_model_ckpt: Union[str, None] = None,
    out_log_file: str = "hrsid_sem_seg_trainer_ddp_dali_tfrecords.log",
) -> None:
    """
    ...
    # -----------------------------------------------
    # initialize DDP, setup logger
    # -----------------------------------------------
    ddp_context = setup_ddp()
    ...
    if is_main_process():
        logger.info(f"Model: {model_name}, model compile method: {model_compile_mode}")
        logger.info(f"DDP training with world_size={ddp_context.world_size}")
        logger.info(
            f"label names for display in experiment logs: {labels_display_logs}"
        )

    path_dir_tmp_ckpt_model = Path(dir_tmp_ckpt_model)
    if is_main_process() and not path_dir_tmp_ckpt_model.is_dir():
    ...
    train_loader = build_dali_tfrecords_loader(
        ...
        device_id=ddp_context.local_rank,
        is_train=True,
        shuffle=True,
        shard_id=ddp_context.rank,
        num_shards=ddp_context.world_size,
    )
    ...
    if is_main_process():
        logger.info("Dali Dataloaders setup complete")
    dist.barrier()
    ...
    if model_name == "convnext_v2_tiny_deeplab_v3+":
        ...
        logger.error(f"unidentified option for model_name={model_name}")
        cleanup_ddp()
        return
    ...
    if file_model_ckpt is not None:
        ...
        checkpoint = torch.load(file_model_ckpt, map_location=ddp_context.device)
        model_state_dict = checkpoint["model_state_dict"]
        model.load_state_dict(model_state_dict)

    # wrap up with DDP
    model.to(ddp_context.device)
    model = DDP(
        model,
    ...
    if is_main_process():
        logger.info("Model wrapping with DDP complete")
    ...
    if loss_fn == "cross_entropy":
        ...
        return
    if is_main_process():
        logger.info("Loss function setup complete")

    # ----------------------------------------------------
    # create an optimizer, learning rate scheduler,
    # and gradient scaler
    # ----------------------------------------------------
    ...
    if is_main_process():
        logger.info("Learning rate scheduler setup complete")

    # create an object for AMP enabled training
    amp_scaler = GradScaler(device="cuda")
    if is_main_process():
        logger.info("Grad Scaler setup complete")

    # ------------------------------------------------------------------------------
    # Use two metrics calculator for train and test separately on respective device
    # ------------------------------------------------------------------------------
    train_metrics = SemSegMetricsCalculator(ddp_context.device, num_classes=num_classes)
    test_metrics = SemSegMetricsCalculator(ddp_context.device, num_classes=num_classes)

    if is_main_process():
        logger.info("Metrics calculator setup complete")

    # --------------------------------------------------------
    # initialize mlflow experiment only on the main process
    # --------------------------------------------------------
    if is_main_process():
        ...
        mlflow_run_tags = {
            ...
            "run": run_name,
            "distributed": "ddp",
            "world_size": str(ddp_context.world_size),
        }

        mlflow.start_run(experiment_id=experiment.experiment_id, tags=mlflow_run_tags)
        ...
        mlflow.log_param(
            "dataset.effective_batch_size", batch_size * ddp_context.world_size
        )
    ...
    try:
        for epoch in range(1, num_epochs + 1):
            ...
            # testing, logging and checkpointing only on rank 0
            if is_main_process():
                logger.info(
                ...
                if (epoch > checkpoint_skip) and (epoch % checkpoint_freq == 0):
                    train_conf_matrix_fig = get_confusion_matrix_figure(
                        train_conf_mat.clone().detach().cpu().numpy(),
                        labels_display_logs,
                    )
                    test_conf_matrix_fig = get_confusion_matrix_figure(
                        test_conf_mat.clone().detach().cpu().numpy(),
                        labels_display_logs,
                    )
